...
const NewsCard: React.FC<NewsCardProps> = ({ title, summary, badge, club, sources, social_embed }) => {
    ...
    return (
        <article
            className="bg-gradient-to-br from-gray-900 via-gray-950 to-gray-900 rounded-3xl shadow-2xl p-6 sm:p-8 flex flex-col gap-4 border border-gray-800 hover:border-green-500 hover:shadow-green-700/40 transition-all hover:scale-[1.03] group focus-within:scale-[1.03] focus-within:border-green-400 outline-none"
            tabIndex={0}
            aria-label={`News: ${title}`}
        >
            <div className="flex flex-col xs:flex-row xs:items-center gap-3 xs:gap-5 mb-2">
                ...
                {club.logo_url ? (
                    <Image
                        ...
                        width={48}
                        height={48}
                        className="w-11 h-11 sm:w-14 sm:h-14 rounded-full bg-gray-700 object-contain border-2 border-green-700 shadow-md group-hover:border-green-400 group-focus-within:border-green-400 transition-all"
                        style={{ objectFit: 'contain' }}
                        loading="lazy"
                        priority={false}
                    />
                ) : (
                    <span className="w-11 h-11 sm:w-14 sm:h-14 rounded-full bg-gray-700 flex items-center justify-center text-2xl sm:text-3xl text-green-500 border-2 border-green-700 shadow-md" aria-label={locale === 'en' ? 'No logo' : 'Kein Logo'}><FaFutbol /></span>
                )}
                <span className="font-extrabold text-xl sm:text-2xl tracking-wide drop-shadow ml-0 xs:ml-2 text-white font-sans flex-1 truncate">
                    {club.name}
                </span>
                <span className="xs:ml-auto mt-1 xs:mt-0 flex items-center gap-2">
                    <button
                        aria-label={fav ? (locale === 'en' ? 'Remove from favorites' : 'Aus Favoriten entfernen') : (locale === 'en' ? 'Mark as favorite' : 'Als Favorit markieren')}
                        onClick={toggleFav}
                        className={`transition-all text-2xl ${fav ? 'text-pink-500 scale-125' : 'text-gray-400 hover:text-pink-400'} focus:outline-none focus:ring-2 focus:ring-pink-400 rounded-full bg-gray-800/60 p-1 shadow-sm`}
            ...
            <p className="text-gray-200 text-base sm:text-lg mb-2 font-medium leading-relaxed">{summary}</p>
            {/* Social Embed */}
